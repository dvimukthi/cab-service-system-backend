...
var conn = mysql.createConnection({
        ...
        return new Promise((resolve) =>{
            ...
            conn.query(SQL, function(err, rows ) {
                ...
                if(rows.length == 0) {
                    ...
                    var branch = new Branch(rows[0].id, 
                        rows[0].location,
                        rows[0].address,
                        rows[0].email,
                        rows[0].phone,
        ...
        return new Promise(async (resolve) =>{
            conn.beginTransaction(async (err) => {
                ...
                var SQL = `UPDATE  go_cheeta.branch 
                        SET 
                        location = '${branch.getLocation()? branch.getLocation() : dbBranch.getLocation()}', 
                        address = '${branch.getAddress()? branch.getAddress() : dbBranch.getAddress()}', 
                        email = '${branch.getEmail()? branch.getEmail() : dbBranch.getEmail()}', 
                        phone = '${branch.getPhone()? branch.getPhone() : dbBranch.getPhone()}'
                        WHERE id = ${id}`;
                conn.query(SQL,  function(err, result ) {
                    if(err) {
                        console.log(err);
  ...
  async function createCustomers(customer){
      return new Promise(async (resolve) =>{
        var user = new User(null, customer.getUserType(),
    ...
    function updateCustomer(customer, id){
        return new Promise((resolve) =>{
            ...
            getCustomersById(id).then((dbCustomer) => {
                var user = new User(dbCustomer.getUserId,
                    "CUSTOMER", 
                    customer.getFirstName() != null ? customer.getFirstName() : dbUser.getFirstName(),
                    customer.getLastName() != null ? customer.getLastName() : dbCustomer.getLastName(),
                    customer.getAddress() != null ? customer.getAddress() : dbCustomer.getAddress(),
                    customer.getEmail() != null ? customer.getEmail() : dbCustomer.getEmail(),
                    customer.getPhoneNumber() != null ? customer.getPhoneNumber() : dbCustomer.getPhoneNumber());
  ...
  function getCustomers() {
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, function(err, rows ) {
            ...
            rows.forEach(r=>{
                var customer = new Customer(r.ID, null, null, 
                r.first_name, r.last_name, r.address,
                r.email, r.phone_number, r.trip_count);
    ...
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, function(err, rows ) {
            if(rows.length == 0) {
                ...
                var customer = new Customer(rows[0].ID, null, null, 
                    rows[0].first_name, rows[0].last_name, rows[0].address,
                    rows[0].email, rows[0].phone_number, rows[0].trip_count);
                resolve(customer);
            }
        });
    });
  ...
  function getDrivers() {
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, async function(err, rows ) {
            ...
            for (const r of rows) {
                var driver = new Driver(r.ID, null, null, 
                r.first_name, r.last_name, r.address,
                r.email, r.phone_number, r.status, r.branch, r.vehicle_id);
  ...
  function getDriverById(id) {
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, async function(err, rows ) {
            ...
            for (const r of rows) {
                var driver = new Driver(r.ID, r.userId, null, 
                r.first_name, r.last_name, r.address,
                r.email, r.phone_number, r.status, r.branch, r.vehicle_id);
    ...
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, function(err, rows ) {
            if(rows.length == 0) {
                ...
                var driver = new Driver(rows[0].ID, null, null, 
                    rows[0].first_name, rows[0].last_name, rows[0].address,
                    rows[0].email, rows[0].phone_number,rows[0].status,  rows[0].branch);
                resolve(driver);
            }
        });
    });
  ...
  function signIn(username, password) {
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, function(err, r ) {
            if(err)
                console.log(err);
            var user = new User(r[0].id, r[0].user_type, r[0].first_name, 
                r[0].last_name, r[0].address,
                r[0].email, r[0].phone_number);
            resolve(user);
        })
    });
  }
  ...
  function getTrips() {
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, function(err, rows ) {
            ...
            rows.forEach(r=>{
                var trip = new Trip(r.id, r.pickup_street, r.pickup_city, 
                r.drop_street, r.drop_city, r.driver_id,
                r.vehicle_id, r.customer_id, r.confirmed);
...
function getTripById(id) {
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, function(err, rows ) {
            ...
            if(rows.length == 0) {
                ...
                var trip = new Trip(rows[0].id, rows[0].pickup_street, rows[0].pickup_city, 
                                    rows[0].drop_street, rows[0].drop_city, rows[0].driver_id,
                                    rows[0].vehicle_id, rows[0].customer_id, rows[0].confirmed);
                resolve(trip);
             }
        })
    });
  ...
  async function getTripsByCustomerId(id) {
    return new Promise((resolve) =>{
        ...
        FROM go_cheeta.trip WHERE customer_id in (SELECT id from go_cheeta.customer WHERE id = '${id}')`;
        var trips = []
        console.log(SQL);
        conn.query(SQL, async function(err, rows ) {
            if(err)
            ...
            for(const r of rows){
                ...
                var trip = new Trip(r.id, r.pickup_street, r.pickup_city, 
                                r.drop_street, r.drop_city, r.driver_id,
                                r.vehicle_id, r.customer_id, r.confirmed, 
                                r.customer_name, r.distance, branch?branch.getLocation():'', r.cost);
        ...
        conn.query(SQL, function(err, rows ) {
            ...
            rows.forEach(r=>{
                var trip = new Trip(r.id, r.pickup_street, r.pickup_city, 
                                r.drop_street, r.drop_city, r.driver_id,
                                r.vehicle_id, r.customer_id, r.confirmed, 
                                r.customer_name, r.distance, r.branch_id, r.cost);
                console.log(trip);
                trips.push(trip);
            });
            console.log(trips);
  ...
  function getUsers() {
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, function(err, rows ) {
            ...
            rows.forEach(r=>{
                var user = new User(r.id, null, null, 
                r.first_name, r.last_name, r.address,
    ...
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, function(err, rows ) {
            if(rows.length == 0) {
                ...
                var user = new User(rows[0].id, null, null, 
                    rows[0].first_name, rows[0].last_name, rows[0].address,
                    rows[0].email, rows[0].phone_number);
                resolve(user);
            }
        });
    });
  ...
  function updateUser(user, id) {
    return new Promise((resolve) =>{
        ...
        getUserById(id).then((dbUser) =>{
            ...
            SET user_type = '${user.getUserType() != null? 
                user.getUserType() : dbUser.getUserType()}',
                first_name = '${user.getFirstName() != null? 
                    user.getFirstName() : dbUser.getFirstName()}',
                last_name = '${user.getLastName() != null? 
                    user.getLastName() : dbUser.getLastName()}',
                address = '${user.getAddress() != null? 
                    user.getAddress() : dbUser.getAddress()}',
                email = '${user.getEmail() != null? 
                    user.getEmail() : dbUser.getEmail()}',
                phone_number = '${user.getPhoneNumber() != null? 
                    user.getPhoneNumber() : dbUser.getPhoneNumber()}' 
            WHERE id = ${id}`
  ...
  async function getVehicles() {
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, async function(err, rows ) {
            ...
            for(const r of rows) {
                var vehicle = new Vehicle(r.id, r.vehicle_name, r.vehicle_type, 
                r.number_plate, r.number_of_seats, r.price,
                r.branch_id);
    ...
    return new Promise((resolve) =>{
        ...
        conn.query(SQL, function(err, rows ) {
            ...
            if(rows.length == 0) {
                ...
                var vehicle = new Vehicle(rows[0].id, rows[0].vehicle_name, rows[0].vehicle_type, 
                    rows[0].number_plate, rows[0].number_of_seats, rows[0].price,
                    rows[0].branch_id);
                }
                resolve(vehicle);
            });
        ...
        getVehicleById(id).then((db) => {
            ...
            conn.beginTransaction((err) => {
                ...
                var SQL = `UPDATE  go_cheeta.vehicle SET 
                                    vehicle_name = '${vehicle.getName()?vehicle.getName():dbVehicle.getName()}',
                                    vehicle_type = '${vehicle.getType()?vehicle.getType():dbVehicle.getType()}',
                                    number_plate = '${ vehicle.getNumberPlate()?vehicle.getNumberPlate():dbVehicle.getNumberPlate()}',
                                    number_of_seats = ${ vehicle.getSeats()?vehicle.getSeats():dbVehicle.getSeats()},
                                    price = ${ vehicle.getPrice()?vehicle.getPrice():dbVehicle.getPrice()},
                                    branch_id = ${ vehicle.getBranchId()?vehicle.getBranchId():dbVehicle.getBranchId()}
                                    WHERE id = ${id}`;
                conn.query(SQL, function (err, result) {
                if (err) {
                    console.log(err);
